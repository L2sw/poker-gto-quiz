...
def run_simulation(hero_combos, villain_combos, board):
    ...
    return (hero_wins + 0.5 * ties) / total if total > 0 else 0.5
...
if st.session_state.game_state == "setup":
    ...
    pos_choice = st.selectbox("自分のポジション(Hero)を選択してください:", ["ランダム"] + POSITIONS_LIST, key="pos_select")
...
# --- 2. クイズ画面 (Flop / Turn / River) ---
    ...
    with st.container(border=True):
        ...
        with col1:
            st.markdown(f"**あなた (Hero)**: `{data['hero_pos']}` ({'OOP' if is_hero_oop(data['hero_pos'], data['villain_pos']) else 'IP'})")
